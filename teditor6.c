...
void expand_editor_enter(editor* E) {
    ...
    if(temp != NULL) {
            insert_gbuf(&E->text[E->y+(E->y_offset*PAGE_SIZE_Y)], temp, E->text[E->y+(E->y_offset*PAGE_SIZE_Y)-1].render_row_size - E->x);
...
void savefile(editor* E, char* filename) {
    ...
    int y = E->y, x = E->x,y_offset = E->y_offset;
...
void move_cursor(char c) {
    ...
    switch(c) {
        ...
        case (char)KEY_RIGHT:
            if(E.y % PAGE_SIZE_Y == 0 && E.x == E.text[E.y+(E.y_offset*PAGE_SIZE_Y)-1].render_row_size)
                move_cursor((char)KEY_DOWN);
            else if(E.x < E.text[E.y+(E.y_offset*PAGE_SIZE_Y)-1].render_row_size) {
                move(E.y, ++E.x);
            }
...
char read_key() {
    ...
    switch(c) {
        ...
        break;
        default:
            insert_char_gbuf(&E.text[E.y+(E.y_offset*PAGE_SIZE_Y)-1], c);
            move(E.y, 0);
            clrtoeol();
            show_gbuf(E, E.y+(E.y_offset*PAGE_SIZE_Y));
            refresh();
